...
## 13. Real-World Example — Adding Dell Nodes n004 & n005 (2026-06-04)

This section documents the live provisioning of Dell compute nodes `n004` and `n005` along with their BMC entries `r004` and `r005`. `n005` is shown as the representative example — `n004` followed the identical pattern with different IPs.

> **Corrections vs. the original log:** The original session log contained two errors worth knowing about:
> - The `r005` BMC entry was accidentally given Panther-cluster postscripts (`panther-scan-nic`, `hardeths`). **BMC/DRAC entries are IPMI tracking nodes only — they do not boot and must not have postscripts.**
> - The `nodeset` command was written without an image name (`nodeset n005 osimage` — incomplete). It must always be `nodeset n005 osimage=<full-image-name>`.

### Step 1 — Define the compute node

```bash
nodeadd n005 groups=dell,alma9
...
chdef -t node n005 \
  ...
  os=alma9.7 \
  xcatmaster=hpcxcat \
  provmethod=alma9.7-x86_64-netbootIB-compute \
  addkcmdline="biosdevname=0 net.ifnames=0"
```

### Step 2 — Configure NIC bonding and VLANs

Dell nodes use the same bond0 pattern as HPE nodes:
...
chdef -t node n005 \
  ...
  nicips.bond0.418=192.168.233.206 \
...
### Step 3 — Define the BMC entry (IPMI tracking only)

```bash
nodeadd r005 groups=mgt
chdef -t node r005 ip=192.168.254.206 mgt=ipmi
```

BMC entries get no postscripts — they are not bootable nodes.

### Step 4 — Register in DNS/hosts, then discover the MAC

```bash
# Register in hosts and DNS first so rpower can resolve r005
makehosts n005,r005
makedns n005

# Start sequential discovery — xCAT will match the first PXE request to n005
nodediscoverstart noderange=n005

# Power on via IPMI to trigger the PXE request
rpower r005 on
```

Watch `/var/log/messages` or the xCAT log for:
```
xcat: Node sequential discovery matching node n005 -> Captured Interface MAC: xx:xx:xx:xx:xx:xx
```
...
> **Why this error happened during the actual session:** After defining the nodes, running `rpower r005 on` immediately threw a resolution error because the management node's hosts file and DNS hadn't been compiled yet. The fix was to run `makehosts` and `makedns` first — exactly the ordering rule from Section 10. Always register DNS/hosts before issuing any power or network commands against a new node.

### Step 5 — Bind the captured MAC and finalize network config

```bash
# Substitute the MAC captured from the discovery log
chdef -t node n005 mac="40:5b:7f:dc:7e:d0" mtm="Dell:R770"

# Rebuild hosts/DNS with the MAC now set, then add to DHCP
makehosts n005,r005
makedns n005
makedhcp -a n005
```
...
```bash
makedhcp -q n005
# Expected: n005: ip-address = 192.168.246.206, hardware-address = 40:5b:7f:dc:7e:d0
```

### Step 6 — Set boot image and provision

```bash
nodeset n005 osimage=alma9.7-x86_64-netbootIB-compute
makegocons n005
rpower n005 reset
rcons n005    # watch the boot; exit: Ctrl-e c .
```

Verify after boot:
```bash
lsdef -t node n005 -i status,updatestatus
# Expected: status=booted, updatestatus=synced
```
